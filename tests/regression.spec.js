...
test('undo/redo stays consistent across many rapid actions', async ({ page }) => {
  ...
  await page.evaluate(() => {
    for (let i = 0; i < 50; i++) {
      window.__LUMEN_TEST.captureUndo('bulk');
  ...
  await page.evaluate(() => { for (let i = 0; i < 50; i++) window.__LUMEN_TEST.performUndo(); });
  const afterUndo = await page.evaluate(() => ({ tasks: state.tasks.length, depth: window.__LUMEN_TEST.undoStack.length }));
  expect(afterUndo.tasks).toBe(10);
  expect(afterUndo.depth).toBe(0);
  // redo all 40 revertible ones
  await page.evaluate(() => { for (let i = 0; i < 50; i++) window.__LUMEN_TEST.performRedo(); });
...
test('module bootstrap: window.LumenLib exists before app boot with all namespaces', async ({ page }) => {
  ...
  expect(shape.keys).toEqual(['constants', 'crypto', 'finance', 'gemini', 'habits', 'helpers', 'merge', 'notes', 'parser', 'persist', 'schedule', 'students', 'tasks', 'vault']);
  expect(errors).toEqual([]);
});
...
test('sync passphrase is salted (v2) after being set', async ({ page }) => {
  ...
  await page.evaluate(async () => {
    if (!window.__LUMEN_TEST.syncMeta.passSalt) window.__LUMEN_TEST.syncMeta.passSalt = window.LumenLib.crypto.randomSaltB64();
    window.__LUMEN_TEST.syncMeta.passHash = await window.LumenLib.crypto.hashPass('test-pass', window.__LUMEN_TEST.syncMeta.passSalt);
    window.__LUMEN_TEST.syncMeta.passHashV = 2;
    window.__LUMEN_TEST.saveSyncMeta();
  });
  await page.reload();
  await page.waitForTimeout(500);
  const meta = await page.evaluate(() => ({ salt: window.__LUMEN_TEST.syncMeta.passSalt, hash: window.__LUMEN_TEST.syncMeta.passHash, v: window.__LUMEN_TEST.syncMeta.passHashV }));
  expect(meta.salt.length).toBeGreaterThan(10);
  expect(meta.hash).toMatch(/^[0-9a-f]{64}$/);
  expect(meta.v).toBe(2);
  await page.evaluate(() => { window.__LUMEN_TEST.syncMeta.passHash = ''; window.__LUMEN_TEST.syncMeta.passSalt = ''; window.__LUMEN_TEST.syncMeta.passHashV = 1; window.__LUMEN_TEST.saveSyncMeta(); });
});

// The sidebar stopped being an <aside> in v123 (it wrapped the primary nav in a
// complementary landmark). Both the CSS rule and the JS lookup keyed off that tag,
// and nothing covered focus mode, so the swap could have silently broken it.
test('focus mode hides the sidebar', async ({ page }) => {
  const errors = [];
...
// A column keeps its scroll position across re-renders. Filtering it down to a list
// SHORTER than the current scroll offset used to leave `first` at 0 while the top
// spacer grew to the whole filtered height — a tall blank gap above the cards, with
// the column scrolled past all of it. Fixed in v126.
test('a column scrolled past its filtered height still shows the cards', async ({ page }) => {
  await page.goto('/#tasks');
  await page.waitForFunction(() => typeof window.state === 'object' && window.state !== null);
  await page.waitForTimeout(500);
  ...
  await page.evaluate(() => {
    const now = Date.now();
    // 200 cards scrolls deep; 20 of them match, so the filtered column is far shorter
    // than the scroll offset we leave behind.
    state.tasks = Array.from({ length: 200 }, (_, i) => ({
      id: 'v' + i, title: (i % 10 === 0 ? 'needle ' + i : 'hay ' + i), desc: '', status: 'today',
      priority: 'med', tags: [], subtasks: [], category: '', due: '', goalId: '',
  ...
  const body = page.locator('.col-body[data-status-body="today"]');
  await body.evaluate((el) => { el.scrollTop = 8000; el.dispatchEvent(new Event('scroll')); });
  await page.waitForTimeout(400);

  await page.fill('#task-q', 'needle');
  await page.waitForTimeout(700); // debounced re-render

  await expect(page.locator('.task-card').first()).toBeVisible();
  const gap = await page.evaluate(() => {
    const b = document.querySelector('.col-body[data-status-body="today"]');
    const spacer = b.firstElementChild;
    const isSpacer = spacer && !spacer.classList.contains('task-card');
    return { topSpacer: isSpacer ? spacer.getBoundingClientRect().height : 0, cards: b.querySelectorAll('.task-card').length };
  });
  expect(gap.cards).toBeGreaterThan(0);
  expect(gap.topSpacer, 'no blank gap taller than the cards above them').toBeLessThan(2000);
});
