...
test('teaching hub weave uses the explicit goal-student link', async ({ page }) => {
  ...
  const hub = await page.evaluate(() => window.__LUMEN_TEST.teachingDashboardHTML());
...
test('student dossier shows linked-goal chips with progress', async ({ page }) => {
  ...
  await page.waitForTimeout(400);
  await page.evaluate(() => window.__LUMEN_TEST.openStudentDossier('s-d'));
  await page.waitForTimeout(400);
...
test('a Kanban task created from an assignment carries the studentId FK', async ({ page }) => {
  ...
  await page.goto('/#students');
  await page.waitForTimeout(400);
  await page.evaluate(() => window.__LUMEN_TEST.openAssignmentModal(null, 'Lale'));
  await page.waitForTimeout(300);
...
test('end-to-end: student ↔ linked goal ↔ income agree across hub, finance, dossier', async ({ page }) => {
  ...
  await page.goto('/#students');
  await page.waitForTimeout(300);
  await page.evaluate(() => window.__LUMEN_TEST.openStudentDossier('s-caner'));
  await page.waitForTimeout(300);
